...
const updateRevokeField = `-- name: UpdateRevokeField :exec
UPDATE refresh_tokens
SET revoked_at = NOW(), updated_at = NOW()
WHERE token = $1
`

func (q *Queries) UpdateRevokeField(ctx context.Context, token string) error {
	_, err := q.db.ExecContext(ctx, updateRevokeField, token)
	return err
}
